class cCeroBarAlert {
    ...
    private ceroBarContainer:Phaser.GameObjects.Container;
    private circleCeroBar:Phaser.GameObjects.Graphics;
    private circleCeroBarBack:Phaser.GameObjects.Graphics;
    private textCeroBarTime:Phaser.GameObjects.BitmapText;

    private circleTween:Phaser.Tweens.Tween;

    constructor(public scene:Phaser.Scene, public data:{status:enumStatus, value:number, maxValue:number, numAlerts:number}) {
    ...
    public refresh(data:{status:enumStatus, value:number, maxValue:number}) {

        var value:number  = Math.floor((data.maxValue - data.value) * 83.33 / 1000 + 1)
